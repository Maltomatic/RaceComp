...
class Resnet_Interpolate_Upscaler(nn.Module):
    def __init__(self, px_shuffle = False, px_shuffle_interpolate = True,
                 px_buffer = True, px_out = False, downsampler = True, input_shape = (3, 112, 112)):
        "Model structure: take 3*112*112 tensor, upscale to 3*224*224 tensor"
        super().__init__()
        self.up_down = downsampler
        ...
        self.double_up_down = nn.Sequential(
            interpolate_resize(scale_factor=2),
            nn.Conv2d(3, 3, kernel_size=3, padding=1, stride=1),
            (nn.Conv2d(3, 3, kernel_size=2, padding=0, stride=2) if self.large else nn.Conv2d(3, 3, kernel_size=3, padding=1, stride=1))
        )
    ...
    def forward(self, x):
        ...
        out = self.up_exit(d0)
        out = self.final(out)
        if(self.up_down or not self.large):
            out = self.double_up_down(out)
...
if __name__ == "__main__":
    ...
    model = Resnet_Interpolate_Upscaler(px_shuffle=False, px_shuffle_interpolate=True, px_buffer = True, px_out=False, input_shape=(3, 56, 56)).to(device)
