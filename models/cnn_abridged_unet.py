    ...
    def __init__(self, output_size=(224,224), px_shuffle = True):
        ...
        if(not px_shuffle):
            ...
            self.dec1 = nn.Sequential(
                nn.Conv2d(256, 64*(2**2), kernel_size=4, padding=4, dilation = 4, stride=1),
                nn.PixelShuffle(2),
                nn.Conv2d(64, 64*(2**2), kernel_size=3, padding=1, stride=1),
                nn.PixelShuffle(2)
            )
            self.conv_up1 = conv_block(64, 3*(2**2))
            self.exit = nn.PixelShuffle(2)
            self.resize = nn.Conv2d(3, 3, kernel_size=3, padding = 1, stride=1)
