...
class Resnet_upscaler(nn.Module):
    def __init__(self, px_shuffle = True, input_shape = (3, 112, 112)):
        ...
        if(px_shuffle):
            self.dec0 = nn.Sequential(
                nn.Conv2d(128, 32*(2**2), kernel_size=3, padding=1, stride=1),
                nn.PixelShuffle(2)
            )
            if(not self.large):
                self.conv_up0 = conv_block(64, 16*(2**2))
                self.double_up = nn.Sequential(
                    nn.Conv2d(64, 3*(2**2), kernel_size=3, padding=1, stride=1),
                    nn.PixelShuffle(2),
                    nn.Conv2d(3, 3*(2**2), kernel_size=3, padding=1, stride=1)
                )
            else:
            ...
            self.dec0 = nn.ConvTranspose2d(128, 32, kernel_size=2, stride=2)
            if(not self.large):
                ...
                self.conv_up0 = conv_block(64, 32)
            self.up_exit = nn.ConvTranspose2d(32, 16, kernel_size=2, stride=2)
    ...
    def forward(self, x):
        ...
        d0 = self.center_crop(d0, x_prep.size())
        d0 = torch.cat([d0, x_prep], dim=1)
        d0 = self.conv_up0(d0)
        # print("    d0 after conv shape:", d0.shape)
        if(not self.large):
            d0 = self.double_up(d0)
            # print("    d0 after double_up shape:", d0.shape)
        
        out = self.up_exit(d0)
        # print("up_exit out shape:", out.shape)
        out = self.final(out)
